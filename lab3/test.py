...
if __name__ == '__main__':
    train_data = pd.read_csv('data/split_words.csv', encoding='utf8')

    train_data['review'] = train_data['review'].apply(lambda x: x[1:-1])

    x_train, x_test, y_train, y_test = train_test_split(
        train_data['review'], train_data['label'], test_size=0.1)

    def cw(x): return int(x)
    x_train = x_train
